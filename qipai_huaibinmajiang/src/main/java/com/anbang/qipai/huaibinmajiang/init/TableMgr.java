...
public class TableMgr {
        ...
        public boolean load()
        {
            for(int i=0; i<9; ++i)
            {
                String path = "/data/app/qipai_hongzhongmajiang/server1/tbl/table_";
                m_check_table[i].load(path + i + ".tbl");
            }

            for(int i=0; i<9; ++i)
            {
                String path = "/data/app/qipai_hongzhongmajiang/server1/tbl/eye_table_";
                m_check_eye_table[i].load(path + i + ".tbl");
            }

            for(int i=0; i<9; ++i)
            {
                String path = "/data/app/qipai_hongzhongmajiang/server1/tbl/feng_table_";
                m_check_feng_table[i].load(path + i + ".tbl");
            }

            for(int i=0; i<9; ++i)
            {
                String path = "/data/app/qipai_hongzhongmajiang/server1/tbl/feng_eye_table_";
